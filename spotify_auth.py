...
from pytify.core.config import read_config
from pytify.core.exceptions import BadRequestError
from pytify.auth.authorization import Authorization
from pytify.auth.auth import get_auth_key
...
def _authorization_code_request(auth_code):
    ...
    response = requests.post(
        config.access_token_url, headers=headers, data=options, timeout=1.5
        )
